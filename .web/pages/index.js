...
export default function Component() {
  ...
  {state.home_state.followers.map((bwbhbjda, i) => (
  <VStack key={i} sx={{"padding": "1em"}}>
  <HStack>
  <Avatar name={bwbhbjda.follower_username} size={`sm`}/>
  <Text>
  {bwbhbjda.follower_username}
</Text>
</HStack>
</VStack>
))}
  ...
  {state.home_state.search_users.map((unexigak, i) => (
  <VStack key={i} sx={{"py": 2, "width": "100%"}}>
  <HStack sx={{"width": "100%"}}>
  <Avatar name={unexigak.username} size={`sm`}/>
  <Text>
  {unexigak.username}
</Text>
  <Spacer/>
  <Button onClick={(_e) => addEvents([Event("state.home_state.follow_user", {username:unexigak.username})], (_e))}>
  <AddIcon/>
</Button>
</HStack>
</VStack>
  ...
  {state.home_state.following.map((gtuyttvu, i) => (
  <VStack key={i} sx={{"padding": "1em"}}>
  <HStack>
  <Avatar name={gtuyttvu.followed_username} size={`sm`}/>
  <Text>
  {gtuyttvu.followed_username}
</Text>
</HStack>
</VStack>
))}
